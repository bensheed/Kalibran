...
export const moveCard = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const lastTransition = await client.query(
            `UPDATE column_transitions
            SET exited_at = NOW()
        ...
        const sequenceResult = await client.query(
            'SELECT COUNT(*) as count FROM column_transitions WHERE job_no = $1 AND column_id = $2',
            [job_no, new_column_id]
        );
        const sequence_number = parseInt(sequenceResult.rows[0].count, 10) + 1;

        // Create a new transition record for the new column
        const newTransition = await client.query(
            `INSERT INTO column_transitions (job_no, inst_id, column_id, entered_at, sequence_number)
            VALUES ($1, $2, $3, NOW(), $4)
            RETURNING *`,
            [job_no, inst_id, new_column_id, sequence_number]
        );
